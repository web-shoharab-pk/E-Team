...
const ViewAllAdmin = () =>{
    const [allSysAdmin, setAllSysAdmin] = useState([] as object[])

    useEffect(() => {
        db.collection("system_admins").get().then((users: any) => {
            let allAdmin = users.docs.map((doc: any) => doc.data());
            
            setAllSysAdmin(allAdmin);
        }).catch((error) => {
    ...
    return (
        <div className="shadow mx-0 lg:mx-8 mt-4 pt-4 pb-16 rounded-lg">
            <h2 className="text-center text-2xl py-3 font-semibold">All System Admins List</h2>
