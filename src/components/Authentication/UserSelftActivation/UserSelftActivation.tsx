...
const UserSelftActivation = () => {
    const [tokenData, setTokenData] = useState({ token: "", email: "", attempt: 0, name: "", expired_at: "", isActivated: false, phone: "", created_by: "",co_id:"" });
    ...
    useEffect(() => {
        if (email && key) {
            db.collection("tokens_user").doc(email).get().then((dataObj: any) => {
                if (dataObj?.exists) {
    ...
    const handleActivate = () => {
        ...
        if (password && password.length >= 8 && re_password && re_password.length >= 8 && password === re_password) {
            const newUserData = { name: tokenData.name, phone: tokenData.phone, email, address: "", photoURL: "", created_by: tokenData.created_by , co_id: tokenData.co_id};
            userRegistration(email, password, newUserData)
                .then((data: any) => {
                    if (!data?.isError) {
                        setIsActivated(true);
                        setError(data);
                        db.collection('tokens_user').doc(email).update({ ...tokenData, isActivated: true }).then(() => {
                ...
                <div className="w-full sm:w-1/2 md:w-1/3 lg:1/4 my-10 py-7 px-3 shadow-md rounded-md border border-gray-100 text-center">
                    ...
                    }
                    {
                        ...
                        <div>
                            ...
                            <a href="/login" className="py-1 px-4 text-white bg-blue-500 rounded-full">User Login</a>
                        </div>
                    }
                </div>
            }
...
export default UserSelftActivation;